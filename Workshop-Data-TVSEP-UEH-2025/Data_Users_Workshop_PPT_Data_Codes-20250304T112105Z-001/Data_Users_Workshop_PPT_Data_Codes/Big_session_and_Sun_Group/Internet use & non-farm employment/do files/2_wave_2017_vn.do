***2017 this do file is used for extracting variable from wave 2017 for data users workshop within program of TVSEP conference
...
* Computer room (change to your working directory)   //you have to change when you use this dofile according to your directory

global datain "C:\Users\nguyettran_L\Dropbox\tvsep conference\TVSEP data-users presentations\internet&non-farm\datain\wave_7_2017_VN"  //this path is for all original data sets which were sent by TVSEP owner

global dataout "C:\Users\nguyettran_L\Dropbox\tvsep conference\TVSEP data-users presentations\internet&non-farm\dataout\dta_2017" // this path is for extracted variables that we cleaned and generated for our study
...
use "$datain\houseclean.dta", replace

***Internet use (yes/no)
tab _x92019
/*

    When your household uses the |
      internet what is the major |
                    device used? |      Freq.     Percent        Cum.
---------------------------------+-----------------------------------
                  1 - Smartphone |        694       36.56       36.56
 2 - Computer (PC and/or laptop) |        123        6.48       43.05
                      3 - Tablet |         17        0.90       43.94
4 - Device in internet cafe/shop |          8        0.42       44.36
  5 - Does not apply (no access) |      1,050       55.32       99.68
             90 - Other, specify |          6        0.32      100.00
---------------------------------+-----------------------------------
                           Total |      1,898      100.00
*/
...
***internet use on productive purposes
gen internet_work= 0 
...
********************************************************MEMBER VARIABLES
use "$datain\memclean.dta", replace
br hhid _x21004 _x21001 _x21005 _x21016 _x21014 _x21015 _x12122 if _x21022==0
replace  _x21022=1 if _x21005==1& _x21022==0
replace  _x12122=_x12122+1 if _x21005==1& _x21016<180

keep if _x21022==1 // keep only if nucleus member

// this two hhid dont have hh head becaused of being died in the reference period
br if QID==41105210301 // 16 days lived only, change hh head to wife
replace _x21005=1 if _x21005==2&QID==41105210301

br if QID==60519230209 // 345 days, so till calculated 
replace _x21005=1 if _x21005==.&QID==60519230209
replace _x21006=2 if _x21006==.&QID==60519230209
replace _x21003=1 if  _x21005==1&QID==60519230209
replace _x21011=1 if  _x21005==1&QID==60519230209
replace _x21004=55 if  _x21005==1&QID==60519230209

***Age //8510obs

gen age = _x21004
lab var age "age of household members, years"
replace age= _x21004a if age==.
...
***household size 
tab _x12122, m //8510 no missing
clonevar HH_size=_x12122

****EDUCATION VARIABLES 
replace _x22014=7 if _x22005==57& _x22014==.
replace _x22014=9 if _x22005==59& _x22014==.
replace _x22014=11 if _x22005==61& _x22014==.
replace _x22014=4 if _x22005==54& _x22014==.
replace _x22014=6 if _x22005==56& _x22014==.
replace _x22014=3 if _x22005==53& _x22014==.
replace _x22014=12 if _x22005==62& _x22014==.
replace _x22014=1 if _x22005==51& _x22014==.
replace _x22014=2 if _x22005==52& _x22014==.
replace _x22014=8 if _x22005==58& _x22014==.
replace _x22014=13 if _x22005==74& _x22014==.
replace _x22014=0 if _x22006==2& _x22014==.
replace _x22014=10 if _x22005==60& _x22014==.
replace _x22014=15 if _x22005==76& _x22014==.
replace _x22014=16 if _x22005==68& _x22014==.
replace _x22014=5 if _x22005==55& _x22014==.
replace _x22014=17 if _x22005==77& _x22014==.
replace _x22014=17 if _x22005==69& _x22014==.
replace  _x22014=0 if  _x22014==.

replace _x22014=12 if hhid==2768&_x21001==4
replace _x22014=15 if hhid==2713&_x21001==3
...
replace mean_schoolyear=0 if mean_schoolyear==. //all childrens
...
***number of non-farm laborers
br hhid HH_size _x21014 
gen nonfarm_labor = 0
...
gen nf_wage =0
replace nf_wage=1 if _x21014==4|_x21014==5|_x21014==7|_x21014==8|_x21015==5|_x21015==7|_x21015==8|_x21015==4
...
***number of phones and televisions
...
replace land_value = land_value*0.0878
label variable land_value "PPP USD"
label variable land_location "All land plots in the village = 1"
label variable HH_land_plot "Total number of land plots"

save "$dataout\land_2017.dta", replace


use "$datain\Landclean.dta", replace

collapse (mean) _x41013 , by(QID)
merge m:m QID using "$dataout\land_2017.dta"
label variable _x41013 "average distance from house (km)"
rename _x41013 average_plot_distance
drop _merge

save "$dataout\4_land_2017.dta", replace


********************************************
*2. MERGING BETWEEN ALL SECTIONS
********************************************
use "$dataout\1_internet.dta", replace
***internet use 
...
**** total assets value per capita PPP FACTOR 0.092
merge 1:1 QID using "$dataout\3_hh_assets.dta"
drop _merge

***Land per capita
merge 1:1 QID using "$dataout\4_land_2017.dta"
drop _merge
...
merge 1:1 QID hhid prov using "$datain\w7_hhinc_vn_v2.dta"
/*
 Result                           # of obs.
    -----------------------------------------
    not matched                           111
...
replace P_x10086=0 if P_x10086<0| P_x10086==.
...
save "$datafinal\household2017.dta", replace
